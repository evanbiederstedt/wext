...
# Create the plot
sns.boxplot(x="Cancer", y="Mutated genes per sample", hue="Type", data=df)
plt.yscale('log')
...
for c in args.cancers:
    all_rates = list(df.loc[df['Cancer'] == c]['Mutated genes per sample'])
    hyper_rates = list(df.loc[(df['Cancer'] == c) & (df['Type'] == "Hypermutator")]['Mutated genes per sample'])
